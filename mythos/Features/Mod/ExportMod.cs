        ...
        public async Task<bool> ExportAsync(int modId, bool gameVersion) {
            ...
            try {
                ...
                if (gameVersion) {
                    ...
                    DirectoryUtilities.Copy(Path.Combine(FilePaths.GetMythsRPFolder, _packs["RP"]), Path.Combine(_tempFolderPath, "RP"), true);

                    if (Directory.Exists(Path.Combine(FilePaths.GetMythsBPFolder, _packs["BP"])))
                        DirectoryUtilities.Copy(Path.Combine(FilePaths.GetMythsBPFolder, _packs["BP"]), Path.Combine(_tempFolderPath, "BP"), true);
                }
                else {

                    if (!Directory.Exists(Path.Combine(_mythFolderPath, _packs["RP"])))
                        throw new Exception("Could not find RP");
                    DirectoryUtilities.Copy(Path.Combine(_mythFolderPath, _packs["RP"]), Path.Combine(_tempFolderPath, "RP"), true);

                    if (Directory.Exists(Path.Combine(_mythFolderPath, _packs["BP"])))
                        DirectoryUtilities.Copy(Path.Combine(_mythFolderPath, _packs["BP"]), Path.Combine(_tempFolderPath, "BP"), true);
                }

                System.IO.File.Create(Path.Combine(_tempFolderPath, "modInfo.json")).Close();

                JsonWriterHelper.WriteJsonFile(Path.Combine(_tempFolderPath, "modInfo.json"), Mod, true);
